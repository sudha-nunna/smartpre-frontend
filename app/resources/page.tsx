...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    ...
    fetch(" https://smartprep-backend-6.onrender.com/api/subjects") 
      .then((res) => res.json())
      .then((data) => {
        setSubjects(data)
        if (data.length > 0) {
    ...
    const fetchResources = async () => {
      try {
        const res = await fetch(" https://smartprep-backend-6.onrender.com/api/resource-types"); // your backend API
